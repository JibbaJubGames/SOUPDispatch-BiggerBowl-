...
public class DispatchDetailGeneratorScript : MonoBehaviour
{
    ...
    private static void SetCallerType()
    {
        if (!emergencyDispatch)
        {
            callerType = Random.Range(1, 5);
            if (callerType == 1)
            {
                PlayerResponses.toThePointCallerSetter = true;
            }
            else if (callerType == 2)
            {
                PlayerResponses.scaredCallerSetter = true;
            }
            else if (callerType == 3)
            {
                PlayerResponses.angryCallerSetter = true;
            }
            else if (callerType == 4)
            {
                PlayerResponses.calmCallerSetter = true;
            }
            else return;
        }
        else return;
